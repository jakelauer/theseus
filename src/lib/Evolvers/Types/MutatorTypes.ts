...
/**
 * Defines the structure and expected behavior of a mutator function. Mutators are functions that
 * receive an object containing the current state data keyed by `TParamName` and potentially
 * additional arguments. They must return either the mutated state data directly or a promise that
 * resolves to it, encapsulated by `SortaPromise`.
 */
export type Mutator<
    TEvolverData,
...
/**
 * Matches the signature of a mutator function, without requiring specific arguments. This is useful
 * for dynamically adding mutators to an evolver instance, passing the arguments through at the time
 * of invocation as-is.
 */
...
/**
 * A union type that can represent either a single mutator function or a nested collection of
 * mutator definitions. This allows for the recursive definition of mutators, supporting complex,
 * nested mutations.
 */
...
/**
 * Constructs a dictionary type from a set of mutator definitions, converting each mutator function
 * into a form that is compatible with the chaining and application mechanisms of the Theseus
 * project.
 */

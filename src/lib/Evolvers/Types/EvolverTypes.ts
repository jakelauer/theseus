 ...
 * It defines interfaces for `mutate` and `evolve` operations, each returning a set of chainable mutators
 * and a method to retrieve these mutators directly.
 *
...
> = {
    /**
     * Initiates a mutation operation on the evolver's data. Unlike `evolve`, `mutate` is tailored for situations where
     * the mutations lead directly to the final state of the data without further chaining.
     *
     ...
     * @returns An object providing a direct way to apply the mutations (`using`) and a method (`getMutators`) to retrieve
     * these mutators. The mutations applied through these mutators conclude the mutation process, returning the final state.
     */
    ...
    /**
     * Initiates an evolution operation on the evolver's data, providing access to a set of chainable mutators.
     * This method is suitable for applying a series of mutations that may include asynchronous operations.
     *
     * @param input The initial state data to evolve.
     * @returns An object containing a set of chainable mutators (`using`) and a method (`getMutators`) to directly retrieve these mutators.
     */
...
/**
 * Type representing the result of a mutate operation on an evolver, providing a fluent API
 * for applying mutations to the state.
 *
...
/**
 * A utility type for representing values that might either be a direct value or a promise resolving to that value.
 * Facilitates handling operations that can be either synchronous or asynchronous.
 *

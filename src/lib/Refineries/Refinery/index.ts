...
/**
 * Represents a refinery for transforming data using a set of forge functions. A refinery
 * encapsulates the logic for applying these transformations, offering a fluent API for defining and
 * executing data refinements.
 *
...
> {
    ...
    /**
     * Initiates the refinement process on the input data using the defined set of forges. This
     * method returns an object allowing further specification of how the data should be refined.
     */
    public refine(input: TForgeableData) {
        const inputClone = cloneDeep(input);
        log.debug(`Cloned input data for refinery: `, inputClone);

        deepFreeze(inputClone);
        log.debug(
            `Ensured input data is immutable for refinery "${this.refineryName}" via deep-freeze`,
        );
        ...
        log.debug(`Created forge set for refinery "${this.refineryName}"`);

        /**
         * Save memory by not duplicating forgeSet on both `into` and `forges`. Why both? Depends on
         * the use-case. Destructuring feels better with `forges`, but `into` looks better with
         * chaining.
         *
    ...
    /**
     * Factory method for creating a new refinery instance. This method initiates the refinery
     * configuration process, allowing the caller to specify the data type to be refined and the
     * forge functions to be used for the refinement. It employs a fluent API design, enabling a
     * step-by-step specification of the refinery's configuration in a clear and intuitive manner.
     *
     * The method returns an object with a `toRefine` method, marking the beginning of the refinery
     * configuration. The `toRefine` method, in turn, expects a callback to define the data type for
     * the refinery. This callback provides another method, `withForges`, which is used to specify
     * the forge functions applicable to the data type. Finally, `withForges` returns the configured
     * refinery instance, ready for use.
     *
     * Usage example:
     *
     * ```typescript
     ...
     * @param definitions An object containing the refinery's name and optionally the data noun. The
     *   data noun specifies the name of the parameter that represents the forgeable data within the
     *   forge functions.
     * @returns An object with a `toRefine` method, marking the start of the refinery configuration
     *   process.
     */
    public static create = <_TRefineryName extends string, _TParamNoun extends string>(
         ...
         * For example, if the name is "shoppingCart", each action will receive an argument entitled
         * "mutableShoppingCart"
         */

...
describe("proxyGet", function() 
{
	...
	{
		const prop = `${CONSTANTS.VERIFICATION.CHECK_PROP_PREFIX}123__propName`;
		const verificationValue = "123";
		const target = {
			[CONSTANTS.VERIFICATION.BASIS_SYMBOL]: verificationValue,
		};
	...
	it("should return true for IS_FROSTY_PROP", function() 
	{
		const target = {};
		const prop = CONSTANTS.IS_FROSTY_PROP;
		const result = proxyGet(target, prop);
		expect(result).to.be.true;
	});
